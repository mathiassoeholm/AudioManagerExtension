...
public class AudioWindow : EditorWindow
{
    ...
    private static bool changesNeedToBeSaved;

    private Vector2 scrollPos;

    private AudioItem itemToRemove;

    public static bool ChangesNeedToBeSaved { get; set; }

    [MenuItem("Window/Audio Manager")]
	static void Init ()
    {
        // Get existing open window or if none, make a new one:
        var window = (AudioWindow)GetWindow(typeof(AudioWindow));
    ...
    void OnDropAudioFile(string filePath)
    {
        ...
        audioItems.Add(new AudioItem {Path = filePath} );
    ...
    void OnGUI()
    {
        ...
        if (GUILayout.Button(ChangesNeedToBeSaved ? "Apply*" : "Apply"))
        {
        ...
        foreach (AudioItem audioItem in audioItems.ToArray())
        {
            ...
            // Play on awake toggle
            audioItem.Is3D = EditorGUILayout.Toggle("3D", audioItem.Is3D);
        }
    ...
    private static void LoadAllAudioItems()
    {
        ...
        for (int i = 0; i < EditorPrefs.GetInt("amountOfAudioItems"); i++)
        {
            audioItems.Add(new AudioItem
            {
                ...
                Is3D = EditorPrefs.GetBool(i + "_Is3D"),
                Volume = EditorPrefs.GetFloat(i + "_Volume"),
            });
        }

        // Don't register changes
